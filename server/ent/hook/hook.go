...
// Mutate calls f(ctx, m).
func (f RecordShFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RecordShMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RecordShMutation", m)
	}
	return f(ctx, mv)
}
